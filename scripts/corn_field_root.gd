...
func _process(delta : float) -> void:
	...
	for i in range(corn_sticks.size()):
		var corn_stick : Node3D = corn_sticks[i]

		if not is_instance_valid(corn_stick):
			continue

		var original_rotation : Vector3 = original_rotations[corn_stick]
		...
		if strongest_bend > 0.0:
			var local_direction : Vector3 = (
				corn_stick.global_transform.basis.inverse()
				* strongest_direction
			)
		...
		corn_stick.rotation = corn_stick.rotation.lerp(
			target_rotation,
			delta * return_speed
		)
