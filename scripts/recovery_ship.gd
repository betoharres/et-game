...
## Altura nativa do ArmBeamMesh (ver RecoveryShip.tscn) antes de esticar o
## cone ate o chao.
const ARM_BEAM_BASE_HEIGHT : float = 6.08
...
func _ready() -> void:
	# Reusa a Saucer (mesma nave que traz o ET do orbital) como visual; a rede
	# de seguranca dela existe para quem anda por cima do deck, o que nao se
	# aplica aqui, e prenderia quem passasse por baixo da nave em voo.
	source_ship.call("set_fall_guard_enabled", false)
...
func _set_tractor_beam(enabled : bool) -> void:
	# Os três braços do raio trator moram ao lado do Hull (não dentro dele): a
	# Saucer não tem holofotes de feixe próprios. Estendemos o volume até a
	# área no solo.
	beam.hide()
	for index : int in range(1, 4):
		var spotlight : SpotLight3D = get_node_or_null(
			"LightArm%d/SpotLight3D" % index
		) as SpotLight3D
		var volume : MeshInstance3D = get_node_or_null(
	...
	# aparece durante a coleta para não formar um triângulo escuro no céu.
		...
		if enabled:
			...
			volume.scale.y = ground_height / ARM_BEAM_BASE_HEIGHT
