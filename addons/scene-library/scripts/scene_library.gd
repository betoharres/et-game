...
func _can_drop_data(at_position: Vector2, data: Variant) -> bool:
	...
	var rec_ext: PackedStringArray = get_supported_asset_extensions()
	...
	for file: String in files:
		...
		if has_asset_path(file) or not is_valid_asset_file(file):
			return false
...
static func get_supported_asset_extensions() -> PackedStringArray:
	var extensions := ResourceLoader.get_recognized_extensions_for_type("PackedScene")
	for extension: String in ResourceLoader.get_recognized_extensions_for_type("ArrayMesh"):
		if not extensions.has(extension):
			extensions.append(extension)
	for extension: String in ["glb", "fbx"]:
		if not extensions.has(extension):
			extensions.append(extension)
	return extensions


static func is_valid_asset_file(path: String) -> bool:
	var resource: Resource = ResourceLoader.load(path)
	return resource is PackedScene or resource is ArrayMesh
...
func create_asset(path: String) -> void:
	assert(is_valid_asset_file(path), "Asset file was not found or has an invalid extension.")
...
func _create_thumb(item: Dictionary[StringName, Variant], callback: Callable) -> void:
	...
	if not is_valid_asset_file(path):
