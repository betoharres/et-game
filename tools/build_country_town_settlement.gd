...
func _build_town() -> void:
	...
	for point: Vector2 in [Vector2(501, 216), Vector2(428, 175), Vector2(486, 316), Vector2(505, 262)]:
	...
	_strip(town, "AvenueWestWalk", [Vector2(448, 171), Vector2(448, 351)], 2.0, Color(0.38, 0.37, 0.32), 0.10)
	_strip(town, "AvenueEastWalk", [Vector2(462, 171), Vector2(462, 351)], 2.0, Color(0.38, 0.37, 0.32), 0.10)
	for z: float in [167.46, 211.0, 238.93, 278.0]:
		for side: float in [-1.0, 1.0]:
			_strip(town, "LocalWalk%d" % town.get_child_count(), [Vector2(465, z + side * 4.1), Vector2(549, z + side * 4.1)], 1.6, Color(0.38, 0.37, 0.32), 0.10)
	...
	for point: Vector2 in [Vector2(508, 174), Vector2(549, 204), Vector2(508, 232), Vector2(549, 271)]:
...
func _build_rural() -> void:
	...
	for index: int in range(1, Layout.RIVER_PATH.size() - 2):
		...
		for step: int in 5:
			for side: float in [-1.0, 1.0]:
				var point: Vector2 = start.lerp(end, (step + 0.5) / 5.0) + normal * side * rng.randf_range(10.0, 14.5)
				if Layout.on_secondary_path(point, 2.0) or _near_main_road(point, 10.0):
					continue
...
func _build_estates(rural: Node3D) -> void:
	...
	for field: Dictionary in Layout.CROP_FIELDS:
		...
		# Portao de 5m no meio de cada lado: entradas nao viram cercas invisiveis.
		_fence_line(rural, rect.position - Vector2(1.2, 1.2), Vector2(rect.end.x + 1.2, rect.position.y - 1.2))
		_fence_line(rural, Vector2(rect.position.x - 1.2, rect.end.y + 1.2), rect.end + Vector2(1.2, 1.2))
		_fence_line(rural, rect.position - Vector2(1.2, 1.2), Vector2(rect.position.x - 1.2, rect.end.y + 1.2))
		_fence_line(rural, Vector2(rect.end.x + 1.2, rect.position.y - 1.2), rect.end + Vector2(1.2, 1.2))
	...
	for index: int in range(1, Layout.RIVER_PATH.size() - 2):
		...
		for step: int in 4:
			for side: float in [-1, 1]:
				var point: Vector2 = start.lerp(end, (step + 0.35) / 4.0) + normal * side * (17.0 + (step % 2) * 3.0)
				if Layout.on_secondary_path(point, 5.0) or _near_main_road(point, 12.0):
					continue
...
func _fence_line(parent: Node, start: Vector2, end: Vector2) -> void:
	...
	if mesh == null:
		return
	var bounds: AABB = mesh.get_aabb()
	var step: float = maxf(bounds.size.x, 1.0)
	var count: int = int(length / step)
	for index: int in count:
		var along: float = (index + 0.5) * step
		if absf(along - length * 0.5) < 3.5:
			continue
		var point: Vector2 = start + axis * along
		var fence: MeshInstance3D = MeshInstance3D.new()
		fence.mesh = mesh
		fence.rotation.y = -axis.angle()
		fence.position = Vector3(point.x, _height(point) - bounds.position.y, point.y) - fence.basis * Vector3(bounds.get_center().x, 0, bounds.get_center().z)
		var world: AABB = fence.transform * bounds
		var footprint: Rect2 = Rect2(world.position.x, world.position.z, world.size.x, world.size.z)
		if not _free_footprint(footprint, 0.2):
			fence.free()
			continue
...
func _build_paths() -> void:
	...
	for path: Dictionary in Layout.SECONDARY_PATHS:
		var color: Color = Color(0.065, 0.072, 0.080) if path["urban"] else Color(0.25, 0.18, 0.10)
		var shoulder_color: Color = Color(0.12, 0.12, 0.11) if path["urban"] else Color(0.29, 0.24, 0.14)
		_strip(paths, String(path["name"]) + "Shoulder", path["points"], float(path["width"]) + 1.3, shoulder_color, 0.026)
		_strip(paths, path["name"], path["points"], path["width"], color, 0.055)
